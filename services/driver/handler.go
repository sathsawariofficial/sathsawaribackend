...
func BookSeatHandler(ctx *gin.Context) {
	...
	uuid, err := BookSeat(ctx, sessionId, driverId, request)
	if err != nil {
	...
	bookingResp := bookSeatResponse(fmt.Sprintf(constants.Success_Info, fmt.Sprintf("%s the seat", action)), uuid)
...
// SaveNotificationSettingsHandler saves the places a driver follows. Ride requests taking
// in one of them are pushed to the driver with their link, and are never kept.
func SaveNotificationSettingsHandler(ctx *gin.Context) {
	...
	driverId := ctx.GetString(constants.User_KEY)
	err := ValidateNotificationSettings(driverId, &request)
	if err != nil {
		logger.LogError(sessionId, "validation error: "+err.Error())
		ctx.JSON(http.StatusBadRequest, utils.APIResponse{
			Code:    http.StatusBadRequest,
	...
	setting, err := SaveNotificationSettings(ctx, sessionId, driverId, request)
	if err != nil {
...
func GetNotificationSettingsHandler(ctx *gin.Context) {
	...
	setting, err := GetNotificationSettings(ctx, sessionId, driverId)
	if err != nil {
